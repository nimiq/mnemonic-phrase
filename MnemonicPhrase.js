class MnemonicPhrase {
    ...
    static _bytesToBinary(bytes) {
        return bytes.map(function(byte) {
            return MnemonicPhrase._lpad(byte.toString(2), '0', 8);
        }).join('');
    }

    static async _deriveChecksumBits(entropy) {
        var ENT = entropy.length * 8;
        var CS = ENT / 32;
        var hash = new Uint8Array(await window.crypto.subtle.digest({ name: 'SHA-256' }, entropy));

        return MnemonicPhrase._bytesToBinary([].slice.call(hash)).slice(0, CS);
    }

    static async entropyToMnemonic(entropy, wordlist) {
        ...
        var checksumBits = await MnemonicPhrase._deriveChecksumBits(entropy);
    ...
    static async mnemonicToEntropy(mnemonic, wordlist) {
        ...
        var newChecksum = await MnemonicPhrase._deriveChecksumBits(entropy);
