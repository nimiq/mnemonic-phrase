...
function arrayToHex(view) {
    ...
    for(var i = 0; i < view.length; i++) {
        result += lpad(view[i].toString(16), '0', 2);
    }
...
function bytesToBinary(bytes) {
    return bytes.map(function(byte) {
        return lpad(byte.toString(2), '0', 8);
    }).join('');
}
...
async function deriveChecksumBits(entropy) {
    ...
    var CS = ENT / 32;
    var hash = new Uint8Array(await window.crypto.subtle.digest({ name: 'SHA-256' }, entropy));

    return bytesToBinary([].slice.call(hash)).slice(0, CS);
}

async function entropyToMnemonic(entropy, wordlist) {
    if(typeof entropy === 'string') entropy = hexToArray(entropy);
    ...
    var checksumBits = await deriveChecksumBits(entropy);

    var bits = entropyBits + checksumBits;
    var chunks = bits.match(/(.{1,11})/g);
    var words = chunks.map(function(binary) {
        var index = binaryToBytes(binary);
...
async function mnemonicToEntropy(mnemonic, wordlist) {
    ...
    var entropyBytes = entropyBits.match(/(.{1,8})/g).map(binaryToBytes);
