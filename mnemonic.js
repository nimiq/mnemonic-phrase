...
async function deriveChecksumBits(entropyBuffer) {
    ...
    var CS = ENT / 32;
    var hash = new Uint8Array(await window.crypto.subtle.digest({ name: 'SHA-256' }, entropyBuffer));

    return bytesToBinary([].slice.call(hash)).slice(0, CS);
}

async function entropyToMnemonic(entropy, wordlist) {
    if(typeof entropy === 'string') entropy = hexToArrayBuffer(entropy);
...
async function mnemonicToEntropy(mnemonic, wordlist) {
    ...
    return arrayBufferToHex(entropy.buffer);
}
